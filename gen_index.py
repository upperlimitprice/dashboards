...
.card{position:relative;display:block;padding:14px 40px 14px 18px;border:1.5px solid #dde3e9;border-radius:11px;margin:10px 0;text-decoration:none;color:#20262e;font-weight:600;background:#fff}
...
function render(){
  ...
  main.innerHTML=state.main.map(k=>{const x=item(k);return `<a class='card' draggable='true' data-k='${x.k}' href='${x.h}'>${x.t}<small>${x.d}</small><button class='x' title='보관함으로' onclick='return rm(event,"${x.k}")'>✕</button></a>`;}).join('')||"<div class='empty'>보관함에서 ＋를 눌러 위젯을 추가하세요</div>";
...
function wireDrag(){
  ...
  box.querySelectorAll('a[draggable]').forEach(el=>{
    ...
    el.addEventListener('dragend',()=>{if(drag)drag.style.opacity=1;drag=null;
      state.main=[...box.querySelectorAll('a[draggable]')].map(x=>x.dataset.k);save();});
