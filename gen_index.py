...
ITEMS = [
    ...
    ("rrg.html", "🔄 RRG 업종 로테이션", "코스피 업종 상대강도 회전(JdK RS-Ratio/Momentum) — 4사분면 주도·둔화·부진·개선, KRX 주간 지수", True),
    ("rrg-stock.html", "🔄 개별종목 RRG (일별)", "코스피 상위100+코스닥50 개별종목 일별 상대강도 회전(JdK) — 4사분면·꼬리10일, 매일 장마감 후 자동 갱신", True),
    ...
    ("screening.html", "📋 수주잔고 & 가동률 스크리닝", "2Q26 기준 — 수주 QoQ/YoY↑ · 가동률 QoQ/YoY↑ · 교집합 7개 리스트 (섹터별 · 인사이트)", True),
    ("hs.html", "🚢 HS코드 월 수출 모니터", "관세청 품목별 수출(USD) — 제품별 실적 선행 프록시 · YoY 모멘텀", True),
    ("exports.html", "📦 주요품목별 수출 대시보드", "264품목 일평균수출·MoM/YoY·판가·분기매출 랭킹·품목별 추이", True),
    ("exports-auto.html", "📦 주요품목별 수출 (관세청 자동)", "관세청 무료 API — HS 10단위 월별 수출·판가·MoM/YoY 자동 수집·매일 갱신 (195품목)", True),
    ("exports-corp.html", "🏢 기업별 수출 추적", "관세청 시군구×HS — 기업·지역별 수출 그룹 추적", True),
    ("trade-stats.html", "🌐 수출입무역통계", "관세청 월별 수출입 — 국가×품목 무역수지", True),
    ("stockmap.html", "🧅 종목 × 무역품목 맵 (HS 검증)", "나쁜양파 맵핑 트렌드 131종목·573품목 — 체크/추정/미충족 구분, 종목별 카드·품목 카드·표, 12개월 막대·공식통계 36개월·KPI 연동 모달", True),
    ("global-trade.html", "🧅 글로벌 수출입 트래커", "미국·일본·브라질·캐나다·필리핀·인니·말레이시아 HS 세부코드 월별 — 시계열·합산·급등·트리맵/히트맵·상대국·HS검색·PPI (최근 36개월)", True),
...
<style>
body{font-family:'Apple SD Gothic Neo','Noto Sans KR',sans-serif;max-width:1080px;margin:40px auto;padding:0 20px;color:#20262e;background:#eceff3}
...
.card{position:relative;display:block;padding:14px 40px 14px 18px;border:1.5px solid #c9d2dc;border-radius:11px;margin:10px 0;color:#20262e;font-weight:600;background:#f9fafc;cursor:pointer}
.card:hover{border-color:#3d5a80;background:#f9fafcbf0}
.card small{color:#6b7683;font-weight:400;display:block;margin-top:3px;font-size:.78rem}
.card small a{color:#3d5a80;text-decoration:none}
...
.rf{position:absolute;bottom:8px;right:10px;border:1px solid #c9d2dc;background:#f9fafc;color:#6b7683;font-size:.72rem;border-radius:7px;padding:2px 7px;cursor:pointer;font-weight:600}
.rf:hover{border-color:#3d5a80;color:#3d5a80}
...
#side{background:#f9fafc;border:1.5px solid #c9d2dc;border-radius:12px;padding:14px 16px;position:sticky;top:16px}
.sitem{display:flex;justify-content:space-between;align-items:center;gap:8px;padding:9px 2px;border-bottom:1px solid #e6ebf1;font-size:.85rem}
.sitem:last-child{border-bottom:0}
.sitem .nm{color:#20262e}
.add{border:1.5px solid #3d5a80;color:#3d5a80;background:#f9fafc;border-radius:8px;font-size:.95rem;width:26px;height:26px;cursor:pointer;font-weight:700;flex:none}
